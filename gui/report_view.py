...
class ReportView:
    ...
    def refresh_all(self):
        ...
        try:
            ...
            self.income_label.config(text=f"总收入：¥{profit.get('total_income', 0):.2f}")
            self.expense_label.config(text=f"总支出：¥{profit.get('total_expense', 0):.2f}")
            
            profit_value = profit.get('profit', 0)
            profit_color = 'green' if profit_value >= 0 else 'red'
            self.profit_label.config(text=f"利润：¥{profit_value:.2f}", foreground=profit_color)
            ...
            for row in sales_data:
                ...
                avg_order_amount = row.get('avg_order_amount', 0)
                
                self.sales_tree.insert('', 'end', values=(
                    type_map.get(order_type, order_type),
                    order_count,
                    f"¥{total_sales:.2f}",
                    f"¥{total_paid:.2f}",
                    f"¥{avg_order_amount:.2f}"
                ))
